...
from lotus.cmds.beta_funcs import (
    default_beta_root_path,
    prepare_lotus_blockchain_log,
    prepare_logs,
...
def print_restart_warning() -> None:
    print("\nRestart the daemon and any running lotus.services for changes to take effect.")
...
def configure(ctx: click.Context, path: Optional[str]) -> None:
    ...
    with lock_and_load_config(root_path, "config.yaml") as config:
        if "beta" not in config:
            ctx.exit("beta test mode is not enabled, enable it first with `lotus.beta enable`")
...
def prepare_submission_cmd(ctx: click.Context) -> None:
    with lock_and_load_config(ctx.obj["root_path"], "config.yaml") as config:
        ...
        if beta_root_path is None:
            ctx.exit("beta test mode not enabled. Run `lotus.beta enable` first.")
    ...
    lotus_blockchain_path = Path(prepare_result / "lotus-blockchain")
    lotus_logs = prepare_logs(plotting_path, prepare_lotus_blockchain_log)
    plotting_logs = prepare_logs(lotus_blockchain_path, prepare_plotting_log)
    ...
    with zipfile.ZipFile(submission_file_path, "w", zipfile.ZIP_DEFLATED) as zip_file:
        files_added = add_files(lotus_logs) + add_files(plotting_logs)

    if files_added == 0:
        submission_file_path.unlink()
        ctx.exit(f"No logs files found in {str(plotting_path)!r} and {str(lotus_blockchain_path)!r}.")
