...
from lotus.cmds.init_funcs import check_keys, lotus_init, lotus_full_version_str
...
from lotus.plotters.plotters import get_available_plotters
...
from lotus.util.lotus_logging import initialize_service_logging
from lotus.util.config import load_config
from lotus.util.errors import KeychainRequiresMigration, KeychainCurrentPassphraseIsInvalid
...
from lotus.util.lock import Lockfile, LockfileError
...
if getattr(sys, "frozen", False):
    name_map = {
        "lotus": "lotus",
        "lotus_data_layer": "start_data_layer",
        "lotus_data_layer_http": "start_data_layer_http",
...
class WebSocketServer:
        ...
        if self.ssl_context.minimum_version is not ssl.TLSVersion.TLSv1_3:
            self.log.warning(
                (
                    ...
                    "A future version of Lotus will require TLS1.3."
                ),
                ssl.OPENSSL_VERSION,
            )
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["lotus", "plotters", plotter]

        command_args.extend(self._common_plotting_command_args(request, ignoreCount))

        if plotter == "chiapos":
            command_args.extend(self._lotus.os_plotting_command_args(request, ignoreCount))
        elif plotter == "madmax":
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service {service_array} with LOTUS_ROOT: {os.environ['LOTUS_ROOT']}")
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    ...
    try:
        with Lockfile.create(daemon_launch_lock_path(root_path), timeout=1):
            log.info(f"lotus-blockchain version: {lotus_full_version_str()}")
