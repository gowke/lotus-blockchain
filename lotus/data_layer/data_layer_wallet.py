...
from lotus.data_layer.data_layer_util import OfferStore, ProofOfInclusion, ProofOfInclusionLayer, StoreProofs, leaf_hash
...
from lotus.wallet.db_wallet.db_wallet_puzzles import (
    ACS_MU,
    ACS_MU_PH,
    GRAFTROOT_DL_OFFERS,
    SINGLETON_LAUNCHER,
...
from lotus.wallet.puzzles.singleton_top_layer_v1_1 import SINGLETON_LAUNCHER_HASH
...
from lotus.wallet.util.merkle_utils import _simplify_merkle_proof
...
class DataLayerWallet:
    ...
    async def create_tandem_xch_tx(
        ...
        lotus.tx = await self.standard_wallet.generate_signed_transaction(
            amount=uint64(0),
        ...
        assert lotus.tx.spend_bundle is not None
        return lotus.tx
    ...
    async def create_update_state_spend(
        ...
        if fee > 0:
            lotus.tx = await self.create_tandem_xch_tx(
                fee, Announcement(current_coin.name(), b"$"), coin_announcement=True
            )
            aggregate_bundle = SpendBundle.aggregate([dl_tx.spend_bundle, lotus.tx.spend_bundle])
            dl_tx = dataclasses.replace(dl_tx, spend_bundle=aggregate_bundle)
            lotus.tx = dataclasses.replace(lotus.tx, spend_bundle=None)
            txs: List[TransactionRecord] = [dl_tx, lotus.tx]
        else:
            txs = [dl_tx]
    ...
    async def delete_mirror(
        ...
        if excess_fee > 0:
            lotus.tx: TransactionRecord = await self.wallet_state_manager.main_wallet.generate_signed_transaction(
                uint64(1),
                new_puzhash,
            ...
            txs = [
                dataclasses.replace(
                    txs[0], spend_bundle=SpendBundle.aggregate([txs[0].spend_bundle, lotus.tx.spend_bundle])
                ),
                dataclasses.replace(lotus.tx, spend_bundle=None),
            ]
