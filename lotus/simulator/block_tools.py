...
from lotus.cmds.init_funcs import create_all_ssl, create_default_lotus.config
...
from lotus.consensus.coinbase import create_farmer_coin, create_pool_coin, create_puzzlehash_for_pk
...
from lotus.plotting.create_plots import PlotKeys, create_plots
...
from lotus.util.config import config_path_for_filename, load_config, lock_config, override_config, save_config
...
class BlockTools:
    ...
    def __init__(
        ...
        if automated_testing:
            ...
            create_default_lotus.config(root_path)
            create_all_ssl(
                root_path,
    ...
    async def setup_keys(self, fingerprint: Optional[int] = None, reward_ph: Optional[bytes32] = None):
        if self.local_keychain:
            ...
            keychain_proxy = await connect_to_keychain_and_validate(
                self.root_path, self.log, user="testing-1.8.0", service="lotus-testing-1.8.0"
            )
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `lotus.keys generate`")
