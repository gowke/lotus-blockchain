...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            lotus_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        lotus_tx = dataclasses.replace(lotus_tx, spend_bundle=spend_bundle)
        await self.standard_wallet.push_transaction(lotus_tx)
    ...
    async def create_tandem_xch_tx(
        ...
        if fee > amount_to_claim:
            ...
            message = None
            for spend in lotus_tx.spend_bundle.coin_spends:
            ...
            lotus_coins = await self.standard_wallet.select_coins(fee)
            selected_amount = sum([c.amount for c in lotus_coins])
            lotus_tx = await self.standard_wallet.generate_signed_transaction(
    ...
    async def generate_unsigned_spendbundle(
        ...
        regular_lotus_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_lotus_to_claim = payment_amount

        need_lotus_transaction = (fee > 0 or regular_chia_to_claim > 0) and (fee - regular_chia_to_claim != 0)
        ...
        for coin in cat_coins:
            if first:
                ...
                if need_lotus_transaction:
                    if fee > regular_lotus_to_claim:
                        lotus_tx, _ = await self.create_tandem_xch_tx(
                            fee, uint64(regular_lotus_to_claim), announcement_to_assert=announcement
                        )
                    ...
                    elif regular_lotus_to_claim > fee:
                        lotus_tx, _ = await self.create_tandem_xch_tx(fee, uint64(regular_chia_to_claim))
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
        ...
        lotus_spend_bundle = SpendBundle([], G2Element())
        if lotus_tx is not None and chia_tx.spend_bundle is not None:
            lotus_spend_bundle = chia_tx.spend_bundle
    ...
    async def generate_signed_transaction(
        ...
        if lotus_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=lotus_tx.confirmed_at_height,
                    created_at_time=lotus_tx.created_at_time,
                    to_puzzle_hash=lotus_tx.to_puzzle_hash,
                    amount=lotus_tx.amount,
                    fee_amount=lotus_tx.fee_amount,
                    confirmed=lotus_tx.confirmed,
                    sent=lotus_tx.sent,
                    spend_bundle=None,
                    additions=lotus_tx.additions,
                    removals=lotus_tx.removals,
                    wallet_id=lotus_tx.wallet_id,
                    sent_to=lotus_tx.sent_to,
                    trade_id=lotus_tx.trade_id,
                    type=lotus_tx.type,
                    name=lotus_tx.name,
                    memos=[],
                )
