...
from lotus.wallet.cat_wallet.cat_info import CATInfo, LegacyCATInfo
...
from lotus.wallet.puzzles.tails import ALL_LIMITATIONS_PROGRAMS
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            lotus.tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        lotus.tx = dataclasses.replace(lotus.tx, spend_bundle=spend_bundle)
        await self.standard_wallet.push_transaction(lotus.tx)
    ...
    async def create_tandem_xch_tx(
        ...
        if fee > amount_to_claim:
            lotus.coins = await self.standard_wallet.select_coins(fee, min_coin_amount=min_coin_amount)
            origin_id = list(lotus.coins)[0].name()
            ...
            message = None
            for spend in lotus.tx.spend_bundle.coin_spends:
            ...
            lotus.coins = await self.standard_wallet.select_coins(fee, min_coin_amount=min_coin_amount)
            selected_amount = sum([c.amount for c in lotus.coins])
            lotus.tx = await self.standard_wallet.generate_signed_transaction(
    ...
    async def generate_unsigned_spendbundle(
        ...
        regular_lotus.to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_lotus.to_claim = payment_amount

        need_lotus.transaction = (fee > 0 or regular_lotus.to_claim > 0) and (fee - regular_lotus.to_claim != 0)
        ...
        for coin in cat_coins:
            if first:
                ...
                if need_lotus.transaction:
                    if fee > regular_lotus.to_claim:
                        lotus.tx, _ = await self.create_tandem_xch_tx(
                            fee,
                            uint64(regular_lotus.to_claim),
                    ...
                    elif regular_lotus.to_claim > fee:
                        lotus.tx, _ = await self.create_tandem_xch_tx(
                            fee, uint64(regular_lotus.to_claim), min_coin_amount=min_coin_amount
                        )
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
        ...
        lotus.spend_bundle = SpendBundle([], G2Element())
        if lotus.tx is not None and lotus.tx.spend_bundle is not None:
            lotus.spend_bundle = lotus.tx.spend_bundle
    ...
    async def generate_signed_transaction(
        ...
        if lotus.tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=lotus.tx.confirmed_at_height,
                    created_at_time=lotus.tx.created_at_time,
                    to_puzzle_hash=lotus.tx.to_puzzle_hash,
                    amount=lotus.tx.amount,
                    fee_amount=lotus.tx.fee_amount,
                    confirmed=lotus.tx.confirmed,
                    sent=lotus.tx.sent,
                    spend_bundle=None,
                    additions=lotus.tx.additions,
                    removals=lotus.tx.removals,
                    wallet_id=lotus.tx.wallet_id,
                    sent_to=lotus.tx.sent_to,
                    trade_id=lotus.tx.trade_id,
                    type=lotus.tx.type,
                    name=lotus.tx.name,
                    memos=[],
                )
