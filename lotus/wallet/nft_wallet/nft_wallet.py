...
from lotus.wallet.nft_wallet.nft_off_chain import delete_off_chain_metadata, get_off_chain_metadata
...
class NFTWallet:
    ...
    async def create_tandem_xch_tx(
        ...
        lotus.coins = await self.standard_wallet.select_coins(fee)
    ...
    async def generate_signed_transaction(
        ...
        unsigned_spend_bundle, lotus.tx = await self.generate_unsigned_spendbundle(
            payments,
            fee,
            coins=coins,
            nft_coin=nft_coin,
        ...
        if lotus.tx is not None and lotus.tx.spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, lotus.tx.spend_bundle])
        ...
        if lotus.tx is not None:
            tx_list.append(lotus.tx)
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            lotus.tx = await self.create_tandem_xch_tx(fee, Announcement(nft_coin.coin.name(), announcement_to_make))
        else:
        ...
        return nft_spend_bundle, lotus.tx
