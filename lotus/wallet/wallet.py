...
class Wallet:
    ...
    async def create_spend_bundle_relative_lotus(self, lotus_amount: int, exclude: List[Coin] = []) -> SpendBundle:
        ...
        if lotus.amount < 0:
            utxos = await self.select_coins(uint64(abs(lotus.amount)), exclude)
        else:
        ...
        lotus.amount = spend_value + lotus.amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries: List[AmountWithPuzzlehash] = [
                    {"puzzlehash": newpuzhash, "amount": uint64(lotus.amount), "memos": []}
                ]
