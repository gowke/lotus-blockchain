...
from lotus.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED, process_config_start_method
from lotus.util.errors import KeychainIsLocked, KeychainProxyConnectionFailure, KeychainIsEmpty, KeychainKeyNotFound
...
from lotus.wallet.util.new_peak_queue import NewPeakItem, NewPeakQueue, NewPeakQueueTypes
from lotus.wallet.util.peer_request_cache import PeerRequestCache, can_use_peer_request_cache
from lotus.wallet.util.wallet_sync_utils import (
...
from lotus.wallet.wallet_weight_proof_handler import get_wp_fork_point, WalletWeightProofHandler
    ...
    def initialize_wallet_peers(self):
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns-introducer.lotus.net"]),
